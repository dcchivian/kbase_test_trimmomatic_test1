package Trimmomatic-test::Trimmomatic-testClient;
...
Trimmomatic-test::Trimmomatic-testClient

=head1 DESCRIPTION


A KBase module: Trimmomatic-test
...
sub new
{
    ...
    my $self = {
	client => Trimmomatic-test::Trimmomatic-testClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$workspace_name is a Trimmomatic-test.workspace_name
$contigset_id is a Trimmomatic-test.contigset_id
$return is a Trimmomatic-test.CountContigsResults
...
# Authentication: required
	...
	method => "Trimmomatic-test.count_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "Trimmomatic-test.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for Trimmomatic-test::Trimmomatic-testClient\n";
    }
    if ($sMajor == 0) {
        warn "Trimmomatic-test::Trimmomatic-testClient version is $svr_version. API subject to change.\n";
    }
}
...
package Trimmomatic-test::Trimmomatic-testClient::RpcClient;
